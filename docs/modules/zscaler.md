...
Threat events are selected by weighted random choice from `_threat_map` (16 entries). Multi-event generators return a Python list of CEF strings (noted below). The `scenario_event` parameter supports `THREAT_BLOCK` (web_threat) and `DATA_EXFIL` (dlp_threat or data_exfil_web_traffic). Named threats — including scenario-only ones like `web_c2_beacon` — are also accessible via the `_NAMED_THREATS` dispatch map, and honor a pinned source host via `context['src_ip']`.
